const newsData = [
    { category: 'Finanza', headline: 'Bitcoin supera la soglia dei 70.000$, analisti divisi sul futuro.' },
    { category: 'Geopolitica', headline: 'Tensioni in aumento nel Mar Cinese Meridionale dopo esercitazioni militari.' },
    { category: 'Tech', headline: 'Rilasciata la nuova CPU quantistica "Aethelred", promette calcoli 1000 volte più veloci.' },
    { category: 'Cybersecurity', headline: 'Il gruppo di hacker "Lazarus" rivendica un attacco da 100 milioni di dollari a un exchange di criptovalute.' },
    { category: 'Economia', headline: 'Inflazione globale in calo per il terzo mese consecutivo, mercati reagiscono positivamente.' },
    { category: 'Geopolitica', headline: 'Accordo di pace storico firmato tra le nazioni di Aethel e Cygnus.' },
    { category: 'Tech', headline: 'Gigante del software "OmniCorp" annuncia una nuova IA per la guida autonoma di livello 5.' },
    { category: 'Cybersecurity', headline: 'Scoperta una vulnerabilità zero-day nel popolare browser "Nexus", patch in arrivo.' },
    { category: 'Finanza', headline: 'Le azioni di "CyberDyne Systems" crollano dopo un report su pratiche di sicurezza deboli.' },
    { category: 'Economia', headline: 'La scarsità di chip semiconduttori continua a colpire l\'industria automobilistica.' },
    { category: 'Geopolitica', headline: 'Disputa commerciale tra USA e Cina si intensifica, nuovi dazi annunciati.' },
    { category: 'Tech', headline: 'Lancio del satellite "StarLink 2.0" completato con successo da SpaceX.' },
    { category: 'Cybersecurity', headline: 'Un\'ondata di attacchi ransomware sta colpendo le infrastrutture sanitarie in Europa.' },
    { category: 'Finanza', headline: 'Monero (XMR) vede un aumento del 30% nel valore dopo l\'adozione da parte di un noto dark market.' },
];
