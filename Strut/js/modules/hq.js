...
// VERSIONE AGGIORNATA: Mostra l'IP del giocatore nel pannello "Computer Status".
...
function renderHqPage() {
    ...
    // NUOVO: Pannello per lo stato della rete personale
    let networkHTML = `
        <div class="hq-stat-card p-4 rounded-lg">
            <h4 class="text-lg font-bold text-indigo-300 mb-2">Stato Rete Personale</h4>
            <div class="text-sm space-y-1">
                <p>Indirizzo IP Pubblico: <span class="font-bold font-mono text-white">${state.identity.realIp}</span></p>
                <p>Stato: <span class="font-bold ${state.identity.isIpDynamic ? 'text-yellow-400' : 'text-green-400'}">${state.identity.isIpDynamic ? 'Dinamico' : 'Statico'}</span></p>
            </div>
        </div>`;
