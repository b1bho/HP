...
function donateToClan() { const amount = parseFloat(document.getElementById('donation-amount').value); if (isNaN(amount) || amount <= 0) { alert("Importo non valido."); return; } if (state.btc < amount) { alert("Non hai abbastanza BTC."); return; } state.btc -= amount; state.clan.treasury += amount * state.btcValueInUSD; updateUI(); renderClanSection(); saveState(); }
...
/**
 * VERSIONE AGGIORNATA: Mostra una nuova sezione per i Dati Intel
 * sopra gli archivi normali.
 */
function renderDataLockerSection() {
    const container = document.getElementById('data-locker-section');
    if (!container) return;

    // Funzione per renderizzare i Dati Intel
