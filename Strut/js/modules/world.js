...
function initGlobe() {
    ...
    const earthTexture = textureLoader.load('https://i.imgur.com/o5uVlTj.jpeg');
    ...
    Object.keys(worldData).forEach(nationName => {
        ...
        nationMesh.userData = { name: nationName, ...nation };
    ...
    renderer.domElement.addEventListener('click', (event) => {
        ...
        const intersects = raycaster.intersectObjects(nationsGroup.children);
        if (intersects.length > 0) {
            showNationPanel(intersects[0].object.userData);
        }
    });
...
function showNationPanel(nation) {
    ...
    panel.innerHTML = `
        ...
        <div class="space-y-4 text-sm mb-6">
            <div>
                <span class="text-gray-300 font-semibold">Livello Sicurezza</span>
                <div class="nation-stat-bar mt-1"><div class="nation-stat-fill bg-blue-500" style="width: ${nation.security}%"></div></div>
            </div>
            <div>
                <span class="text-gray-300 font-semibold">Economia</span>
                <div class="nation-stat-bar mt-1"><div class="nation-stat-fill bg-green-500" style="width: ${nation.economy}%"></div></div>
            </div>
        </div>

        <div class="mb-6">
            <h3 class="font-semibold mb-2 text-indigo-300">Vulnerabilità Note</h3>
            <ul class="list-disc list-inside text-gray-300 text-sm space-y-1">
                ${nation.vulnerabilities.map(v => `<li>${v}</li>`).join('')}
            </ul>
        </div>

        <div class="flex-grow">
            <h3 class="font-semibold mb-3 text-indigo-300">Seleziona Target</h3>
            <div id="target-list" class="space-y-2 mb-4">
                ${nation.targets.map(t => `
                    <div class="target-item p-3 rounded-lg cursor-pointer" data-target-id="${t.id}">
                        <h4 class="font-bold text-base">${t.name}</h4>
                        <p class="text-sm text-gray-400">${t.type}</p>
                    </div>
                `).join('')}
            </div>
        </div>
        ...
        <div id="attack-section" class="hidden mt-auto pt-4 border-t border-gray-600">
            <h3 class="font-semibold mb-2 text-indigo-300">Prepara Attacco</h3>
            <div class="mb-3">
    ...
    // Rimuove la classe -translate-x-full per far apparire il pannello
    ...
    panel.querySelectorAll('.target-item').forEach(item => {
        item.addEventListener('click', () => {
            selectedTarget = nation.targets.find(t => t.id === item.dataset.targetId);
